...
"""
    ...
    try:
        response = await asyncio.wait_for(
            ...
            timeout=420.0  # 7 minute timeout — allows headroom for concurrent tasks
        )
...
async def final_report_generation(state: AgentState):
    ...
    # Check if research was aborted by circuit breaker
    if state.get("aborted", False):
        print("\n--- [NODE: final_report_generation] ---")
        print("[Final Report] SKIPPED — research was aborted by circuit breaker (API quota exhausted)")
        raise RuntimeError("Research aborted: all subagents failed with API quota errors. Check your API billing/quota.")
